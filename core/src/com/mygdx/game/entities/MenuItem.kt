...
class MenuItem(x: Float, y: Float, text: String, font: BitmapFont) {
    ...
    fun update(dt: Float, cam: Camera, viewport: Viewport) {
        ...
        listener?.let {
            ...
            val location = Vector3(InputKeys.screenX.toFloat(), InputKeys.screenY.toFloat(), 0f)
            ...
            if (InputKeys.isMousePressed(InputKeys.MouseKey.LEFT) && boundingRect.contains(location.x, location.y)) {
                it.onClick(this)
            }
        }
    }
