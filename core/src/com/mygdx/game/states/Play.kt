 ...
 */
class Play(gsm: GameStateManager): GameState(gsm), InputTouchPad.Touchable {
    ...
    private var touchPad: InputTouchPad? = null
    ...
    init {
        // set font
        val fontGen = BitmapFont()
//        FreeTypeFontGenerator(Gdx.files.internal("fonts/Hyperspace Bold.ttf"))
//        val freeTypeFontParams = FreeTypeFontGenerator.FreeTypeFontParameter()
//        freeTypeFontParams.size = 20
        font = BitmapFont()
//                fontGen.generateFont(freeTypeFontParams)
...
//        fontGen.dispose()
        ...
        if (Gdx.app.type == Application.ApplicationType.iOS ||
            ...
            // touch pad
            touchPad = InputTouchPad(120f, 120f, 90f, 30f)?.also {
                it.listener = this
            }
    ...
    override fun handleInput(dt: Float) {
        // turn left
        if (( (InputKeys.isButtonDown(InputKeys.ButtonKey.LEFT)) ||
        ...
        // turn right
        if (( (InputKeys.isButtonDown(InputKeys.ButtonKey.RIGHT)) ||
        ...
        // go forward
        if (( (InputKeys.isButtonDown(InputKeys.ButtonKey.UP)) ||
        ...
        // shoot
        if (( (InputKeys.isButtonPressed(InputKeys.ButtonKey.SPACE)) ||
    ...
    override fun onTouchPadTouch(pad: InputTouchPad, radians: Float) {
        touchPadAngle = radians
    }

    override fun onTouchPadCancel(pad: InputTouchPad, lastKnownRadians: Float) {
